...
@section('content')
    <div class="col-md-8 mx-auto my-5">
        <div class="card shadow-lg border-0 rounded-3">
            <div class="card-header bg-warning text-dark text-center fw-bold fs-5 rounded-top">
                Modifier le produit : {{ $produits->name }}
            </div>
            <div class="card-body p-4">
                <form action="{{ route('admin.Produits.update', $produits->id) }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <select name="category" class="form-select @error('categories') is-invalid @enderror">
                            <option value="" hidden>-- Sélectionner la catégorie --</option>
                            @foreach ($categories as $item)
                                <option value="{{ $item->id }}"
                                    {{ $produits->category_id == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" rows="5" class="form-control @error('description') is-invalid @enderror">{{ $produits->description }}</textarea>
                        @error('description')
                            <span class="text-danger small">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" name="prix" value="{{ $produits->prix }}"
                    ...
                    <div class="mb-4">
                        ...
                        <div class="border border-2 rounded-3 p-4 text-center position-relative bg-light"
                            ...
                            <p class="mb-0 text-muted">Glissez-déposez une nouvelle image ou cliquez pour remplacer</p>
                        ...
                        <div class="mt-3 text-center">
                            <img id="imagePreview" src="{{ $produits->image ? asset('/' . $produits->image) : '#' }}"
                                alt="Aperçu de l'image"
                                class="img-fluid rounded shadow-sm {{ $produits->image ? '' : 'd-none' }}"
